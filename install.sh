...
apt-get install -y lame eject id3 id3v2 eyed3 normalize-audio vorbisgain mkcue mp3gain libdata-dump-perl flac glyrc abcde
cp 99-cd-audio-processing.rules /etc/udev/rules.d/.
cp ripit.sh /usr/local/bin/.
cp ripit.service /etc/systemd/system/.
cp ripweb.service /etc/systemd/system/.
